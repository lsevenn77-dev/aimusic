...
export function businessFooterHTML(){
  return `<div class="footer-heading"><a href="/#home" aria-label="AIFECT 홈"><span class="aifect-lockup"><span class="aifect-wordmark">AIFECT</span><span class="aifect-tagline">AI와 음악의 만남</span></span></a></div><p class="footer-product">전체곡 무료 감상 · <a href="/pricing" target="_blank" rel="noopener">Premium 월 ${PREMIUM.price.toLocaleString('ko-KR')}원</a> (부가세 포함)</p><nav class="policy-links" aria-label="서비스 정책">${policyLinks.map(([url,label])=>`<a href="${url}" target="_blank" rel="noopener"${url==='/privacy'?' class="privacy-link"':''}>${label}</a>`).join('')}</nav>${businessDetailsHTML()}<p class="footer-copyright">© ${POLICY_VERSION.slice(0,4)} ${BUSINESS.name}. AIFECT.</p>`;
}
