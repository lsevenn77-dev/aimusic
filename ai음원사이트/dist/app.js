...
function cover(t,cls='album-art'){return `<div class="${cls} cover cover-${t.has_cover?'custom':'lime'}">${t.has_cover?`<img decoding="async" src="/media/${esc(t.id)}/cover?v=${esc(t.cover_version||'original')}" alt="${esc(t.title)} 커버" loading="lazy">`:`<span class="cover-word">${esc((t.title||'A').slice(0,2))}</span>`}</div>`;}
...
function card(t){return `<article class="album-card"><div class="album-art-wrap"><a href="#song/${t.id}">${cover(t)}</a><button class="card-play" data-play="${t.id}" aria-label="${esc(t.title)} 재생">${icon('play')}</button><button class="card-more" data-track-menu="${t.id}" aria-label="${esc(t.title)} 더보기">⋯</button></div><a class="album-title" href="#song/${t.id}">${esc(t.title)}</a><a class="album-artist" href="#artist/${t.artist_id}">${esc(t.artist)} <b class="ai-mini">AI</b></a><p class="album-producer">Produced by <a href="#producer/${t.producer_id}">${esc(t.producer)}</a></p><span class="album-meta">${esc(t.genre)} · ${time(t.duration)}</span>${trackStats(t)}</article>`;}
...
async function refreshLibrary(){library=me?await api('/api/library'):{likes:[],playlists:[],follows:[]};remember(library.likes);setMembership(library.membership);}
